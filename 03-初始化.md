...
`_c`,`$createElement`内部调用了同一个函数`createElement`，查看注释可了解每个参数定义，差别在于最后一个参数`alwaysNormalize`的区别，`Normalize`是`Vue`内部的标准化流程，由于`Vue`兼容许多写法，但内部需要统一处理。

上述两个函数主要在生成的`render`函数中调用，同时我们也可以在组件内通过`this.$createElement`动态渲染组件。
